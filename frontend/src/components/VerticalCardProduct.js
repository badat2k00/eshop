...
import deleteFavoriteProduct from "../helpers/deleteFavoriteProduct";
...
const VerticalCardProduct = ({ category, heading }) => {
  ...
  const toggleFavorite = (e,id,isFavorite) => {
    setFavorite((prevFavorites) => ({
      ...prevFavorites,
      [isFavorite]: !prevFavorites[isFavorite],
    }));
    if(!isFavorite){
      handleAddToFavorite(e,id)
    }else{
      handleDeleteFavoriteProduct(e,id)
    }
  };
  ...
  const handleDeleteFavoriteProduct =async(e,id)=>{
    await deleteFavoriteProduct(e,id)
  }
          ...
          : data.map((product, index) => {
              return (
                ...
                <div className='bg-slate-200 h-48 p-4 min-w-[280px] md:min-w-[145px] flex justify-center items-center relative'>
                    <MdOutlineFavorite 
                        className={`absolute top-2 right-3 z-30 ${favorite[product?.isFavorite] ? 'text-rose-500':'text-black'} cursor-pointer`}
                        onClick={(e)=>{toggleFavorite(e,product?._id,product?.isFavorite)}}
                    />
                    ...
                    <Link to={"product/"+product?._id} className='w-full h-full absolute top-0 left-0 z-10'></Link>
                ...
                <div className='p-4 grid gap-3'>
                    ...
                    <div className='flex gap-3'>
                        ...
                        <h1>{product?.isFavorite?"Yes":"No"}</h1>
                    </div>
